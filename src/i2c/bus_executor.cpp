...

    {
        ...
        while (!pending.empty()) {
            ...
            safe_set_promise(task->promise,
                             make_status(StatusCode::Cancelled, "executor stopped before task execution"));
            ++metrics_.cancelled;
        }
    }
...
Status BusExecutor::submit(const std::string &job_name, std::chrono::milliseconds timeout, Job job) {
    ...
    if (timeout.count() > 0) {
        ...
        if (wait_status != std::future_status::ready) {
            ...
            return make_status(StatusCode::DeadlineExceeded, "job '" + job_name + "' timed out");
        }
    }
...
void BusExecutor::worker_loop() {
    while (true) {
        ...
        if (task->timed_out.load()) {
            safe_set_promise(task->promise,
                             make_status(StatusCode::Cancelled, "task skipped because caller timed out"));
            std::lock_guard<std::mutex> lock(mutex_);
            ++metrics_.cancelled;
            continue;
        }
