...
/**
 * @file health.cpp
 * @brief Health and readiness projection derived from EZO runtime sampling
 * state.
 */
...
ProviderHealth make_provider_health(const runtime::RuntimeState &state) {
    ...
    (*health.mutable_metrics())["active_devices"] = std::to_string(state.active_devices.size());
    (*health.mutable_metrics())["excluded_devices"] = std::to_string(state.excluded_devices.size());
...
std::vector<DeviceHealth> make_device_health(const runtime::RuntimeState &state, bool include_excluded) {
    std::vector<DeviceHealth> devices;
    devices.reserve(state.active_devices.size() + (include_excluded ? state.excluded_devices.size() : 0));
    ...
    for (const auto &device : state.active_devices) {
        ...
        (*health.mutable_metrics())["sample_success_count"] = std::to_string(device.sample.success_count);
        (*health.mutable_metrics())["sample_failure_count"] = std::to_string(device.sample.failure_count);
        (*health.mutable_metrics())["call_success_count"] = std::to_string(device.call_success_count);
        (*health.mutable_metrics())["call_failure_count"] = std::to_string(device.call_failure_count);
...
void populate_wait_ready(const runtime::RuntimeState &state, WaitReadyResponse &out) {
    ...
    (*out.mutable_diagnostics())["active_device_count"] = std::to_string(state.active_devices.size());
    (*out.mutable_diagnostics())["excluded_device_count"] = std::to_string(state.excluded_devices.size());
