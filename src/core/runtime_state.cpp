...
FunctionSpec *
add_function_spec(anolis::deviceprovider::v1::CapabilitySet &caps,
                  uint32_t function_id, const char *name,
                  const char *description,
...
void add_arg_spec(FunctionSpec *function, const char *name,
...
void add_result_spec(FunctionSpec *function, const char *name,
                     anolis::deviceprovider::v1::ValueType type,
                     const char *description) {
  if (function == nullptr) {
    return;
...
void add_safe_function_specs(anolis::deviceprovider::v1::CapabilitySet &caps) {
  FunctionSpec *find_fn = add_function_spec(
      caps, kFunctionFind, "find",
      "Blink the device LED to help identify physical hardware on the bus.",
      kCategoryActuate, false);
  add_result_spec(find_fn, "accepted", kValueTypeBool,
                  "true when the command is accepted by the provider");

  FunctionSpec *set_led_fn = add_function_spec(
      caps, kFunctionSetLed, "set_led",
      "Enable or disable the device status LED.", kCategoryConfig, true);
  add_arg_spec(set_led_fn, "enabled", kValueTypeBool,
               "LED state (true=on, false=off)", true);
  add_result_spec(set_led_fn, "enabled", kValueTypeBool,
                  "Echoed requested LED state");
  add_result_spec(set_led_fn, "accepted", kValueTypeBool,
                  "true when the command is accepted by the provider");

  FunctionSpec *sleep_fn = add_function_spec(
      caps, kFunctionSleep, "sleep",
      "Put the device into low-power sleep mode.", kCategoryConfig, true);
  add_result_spec(sleep_fn, "accepted", kValueTypeBool,
                  "true when the command is accepted by the provider");
}
...
i2c::Status status_from_ezo_result(ezo_result_t result,
...
std::unique_ptr<i2c::ISession> make_session(const ProviderConfig &config) {
  ...
  return std::make_unique<i2c::LinuxSession>(config.bus_path, config.timeout_ms,
                                             config.retry_count);
#else
  return std::make_unique<i2c::NoopSession>(config.bus_path);
#endif
...
std::vector<ActiveDevice>::iterator
find_active_device_unlocked(std::vector<ActiveDevice> &devices,
                            const std::string &device_id) {
  return std::find_if(devices.begin(), devices.end(),
                      [&device_id](const ActiveDevice &device) {
                        return device.spec.id == device_id;
                      });
}

anolis::deviceprovider::v1::CapabilitySet
build_capabilities(const ProviderConfig &config, EzoDeviceType type) {
  ...
  for (size_t i = 0; i < def_count; ++i) {
    ...
    signal->set_poll_hint_hz(1000.0 /
                             static_cast<double>(sample_period_ms(config)));
...
build_descriptor(const ProviderConfig &config, const DeviceSpec &spec) {
...
void fill_mock_identity(int address, ezo_device_info_t *info) {
  ...
  const ezo_product_metadata_t *metadata =
      ezo_product_get_metadata(info->product_id);
  if (metadata != nullptr && metadata->vendor_short_code != nullptr) {
    std::snprintf(info->product_code, sizeof(info->product_code), "%s",
...
i2c::Status probe_identity_real(i2c::BusExecutor &executor,
  ...
  if (info_out == nullptr) {
    return make_status(i2c::StatusCode::InvalidArgument,
                       "probe requires output info");
  }

  ezo_device_info_t info{};
  const ezo_product_id_t expected_product =
      expected_product_for_type(spec.type);
  const int timeout_ms = std::max(config.timeout_ms, 2000);

  const i2c::Status status = executor.submit(
      "startup_probe:" + spec.id, std::chrono::milliseconds(timeout_ms),
      [&](i2c::ISession &session) {
        i2c::EzoDeviceBinding binding;
        i2c::Status bind_status = i2c::bind_ezo_i2c_device(
            session, static_cast<uint8_t>(spec.address), binding);
        if (!bind_status.is_ok()) {
          return bind_status;
        }

        ezo_timing_hint_t hint{};
        const ezo_result_t send_result = ezo_control_send_info_query_i2c(
            &binding.device, expected_product, &hint);
        ...
        const ezo_result_t read_result =
            ezo_control_read_info_i2c(&binding.device, &info);
...
void set_signal_value(std::vector<SignalSample> &signals, size_t index,
                      double value) {
...
void set_signal_unavailable(std::vector<SignalSample> &signals, size_t index,
...
i2c::Status
read_sample_from_bound_device(ezo_i2c_device_t *device, const DeviceSpec &spec,
                              std::vector<SignalSample> *signals_out) {
  ...
  case EzoDeviceType::Ec: {
    ...
    if ((reading.present_mask & EZO_EC_OUTPUT_CONDUCTIVITY) != 0) {
      ...
      set_signal_unavailable(*signals_out, 0,
                             "conductivity output disabled on device");
    }
    ...
    if ((reading.present_mask & EZO_EC_OUTPUT_SALINITY) != 0) {
      ...
      set_signal_unavailable(*signals_out, 2,
                             "salinity output disabled on device");
    }
    if ((reading.present_mask & EZO_EC_OUTPUT_SPECIFIC_GRAVITY) != 0) {
      set_signal_value(*signals_out, 3, reading.specific_gravity);
    } else {
      set_signal_unavailable(*signals_out, 3,
                             "specific gravity output disabled on device");
    }
    return i2c::Status::ok();
  }
  case EzoDeviceType::Do: {
    ...
    if ((reading.present_mask & EZO_DO_OUTPUT_PERCENT_SATURATION) != 0) {
      ...
      set_signal_unavailable(*signals_out, 1,
                             "saturation output disabled on device");
    }
    return i2c::Status::ok();
  }
  case EzoDeviceType::Hum: {
  ...
  return make_status(i2c::StatusCode::Internal,
                     "unsupported device type in sample read");
}

void build_mock_sample(const DeviceSpec &spec, uint64_t sequence,
                       std::vector<SignalSample> *signals_out) {
  initialize_signal_samples(spec.type, signals_out);
  if (signals_out == nullptr) {
    return;
...
void initialize(const ProviderConfig &config) {
  ...
  if (!start_status.is_ok()) {
    state.startup_message =
        "startup failed to initialize I2C executor: " + start_status.message;
    std::lock_guard<std::mutex> lock(g_mutex);
  ...
  for (const DeviceSpec &spec : config.devices) {
    ...
    if (!probe_status.is_ok()) {
      state.excluded_devices.push_back(
          ExcludedDevice{spec, probe_status.message});
      continue;
    }
    ...
    if (info.product_id != expected_product) {
      ...
      if (const ezo_product_metadata_t *meta =
              ezo_product_get_metadata(info.product_id)) {
      ...
      state.excluded_devices.push_back(
          ExcludedDevice{spec, "type mismatch: configured " + expected +
                                   ", detected " + actual});
      continue;
    }
    ...
    device.sample.signals.resize(
        static_cast<size_t>(device.capabilities.signals_size()));
    (*device.descriptor.mutable_tags())["ezo_product_code"] =
        device.startup_product_code;
    (*device.descriptor.mutable_tags())["ezo_firmware"] =
        device.startup_firmware_version;
...
i2c::Status refresh_device_sample(const std::string &device_id) {
  ...
  const int timeout_ms =
      std::max(config.timeout_ms, sample_period_ms(config) + 1500);
  ...
  const i2c::Status status = submit_i2c_job(
      "sample:" + device_id, std::chrono::milliseconds(timeout_ms),
      [&](i2c::ISession &session) {
        i2c::EzoDeviceBinding binding;
        i2c::Status bind_status = i2c::bind_ezo_i2c_device(
            session, static_cast<uint8_t>(spec.address), binding);
        ...
        return read_sample_from_bound_device(&binding.device, spec,
                                             &new_signals);
      });
