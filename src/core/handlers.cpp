...
from_proto_timestamp(const google::protobuf::Timestamp &timestamp) {
...
to_proto_timestamp(std::chrono::system_clock::time_point time_point) {
...
const runtime::ActiveDevice *find_device(const runtime::RuntimeState &state,
  ...
  auto it =
      std::find_if(state.active_devices.begin(), state.active_devices.end(),
...
int find_signal_index(const runtime::ActiveDevice &device,
...
const FunctionSpec *find_function_by_id(const runtime::ActiveDevice &device,
...
bool validate_call_args(uint32_t function_id, const ValueMap &args,
                        bool *set_led_enabled, std::string *error_out) {
  ...
  if (function_id == runtime::kFunctionFind ||
      function_id == runtime::kFunctionSleep) {
    if (!args.empty()) {
...
bool resolve_call_timeout(const CallRequest &request, int default_timeout_ms,
...
i2c::Status execute_safe_call(const runtime::RuntimeState &state,
  ...
  const ezo_product_id_t product_id =
      expected_product_for_type(device.spec.type);
  if (product_id == EZO_PRODUCT_UNKNOWN) {
    return make_i2c_status(
        i2c::StatusCode::Internal,
  ...
  return runtime::submit_i2c_job(
      "call:" + std::to_string(function_id) + ":" + device.spec.id, timeout,
      [&](i2c::ISession &session) {
        i2c::EzoDeviceBinding binding;
        i2c::Status bind_status = i2c::bind_ezo_i2c_device(
            session, static_cast<uint8_t>(device.spec.address), binding);
        ...
        case runtime::kFunctionFind:
          result =
              ezo_control_send_find_i2c(&binding.device, product_id, &hint);
          break;
        case runtime::kFunctionSetLed:
          result = ezo_control_send_led_set_i2c(
              &binding.device, product_id, set_led_enabled ? 1U : 0U, &hint);
          break;
        case runtime::kFunctionSleep:
          result =
              ezo_control_send_sleep_i2c(&binding.device, product_id, &hint);
          break;
        default:
          return make_i2c_status(i2c::StatusCode::InvalidArgument,
...
void populate_signal_value(const runtime::RuntimeState &state,
                           const runtime::ActiveDevice &device,
                           size_t signal_index, SignalValue *value) {
  if (value == nullptr) {
    return;
  }
  ...
  const auto &signal_spec =
      device.capabilities.signals(static_cast<int>(signal_index));
  value->set_signal_id(signal_spec.signal_id());
  value->mutable_value()->set_type(
  ...
  *value->mutable_timestamp() = to_proto_timestamp(device.sample.sampled_at);
  ...
  if (signal_sample != nullptr && !signal_sample->available) {
    ...
    (*value->mutable_metadata())["unavailable_reason"] =
        signal_sample->unavailable_reason;
  }
}
...
void handle_read_signals(const ReadSignalsRequest &request,
  ...
  if (request.has_min_timestamp()) {
    ...
    if (!device->sample.has_sample ||
        device->sample.sampled_at < min_timestamp) {
      needs_refresh = true;
    }
  }

  if (needs_refresh) {
    const i2c::Status refresh_status =
        runtime::refresh_device_sample(request.device_id());
    if (!refresh_status.is_ok()) {
      logging::warning("read_signals refresh failed for '" +
                       request.device_id() + "': " + refresh_status.message);
      state = runtime::snapshot();
      device = find_device(state, request.device_id());
      if (device == nullptr || !device->sample.has_sample) {
        set_status(response, map_i2c_status_code(refresh_status.code),
  ...
  if (has_min_timestamp && device->sample.sampled_at < min_timestamp) {
    set_status(response, Status::CODE_DEADLINE_EXCEEDED,
...
void handle_call(const CallRequest &request, Response &response) {
  ...
  if (!validate_call_args(function_spec->function_id(), request.args(),
                          &set_led_enabled, &arg_error)) {
    runtime::record_call_result(request.device_id(), function_spec->name(),
                                false, arg_error);
  ...
  if (!resolve_call_timeout(request, state.config.timeout_ms, &timeout,
                            &timeout_error)) {
    runtime::record_call_result(request.device_id(), function_spec->name(),
                                false, timeout_error);
  ...
  const i2c::Status call_status = execute_safe_call(
      state, *device, function_spec->function_id(), set_led_enabled, timeout);
  if (!call_status.is_ok()) {
    runtime::record_call_result(request.device_id(), function_spec->name(),
                                false, call_status.message);
    logging::warning("call failed for device '" + request.device_id() +
                     "' function '" + function_spec->name() +
                     "': " + call_status.message);
    set_status(response, map_i2c_status_code(call_status.code),
               call_status.message);
    return;
  }
