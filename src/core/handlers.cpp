...
std::chrono::system_clock::time_point from_proto_timestamp(const google::protobuf::Timestamp &timestamp) {
    ...
    const auto duration = std::chrono::duration_cast<std::chrono::system_clock::duration>(seconds + nanos);
...
bool is_mock_mode(const runtime::RuntimeState &state) { return has_prefix(state.config.bus_path, "mock://"); }
...
bool resolve_call_timeout(const CallRequest &request, int default_timeout_ms, std::chrono::milliseconds *timeout_out,
...
void handle_read_signals(const ReadSignalsRequest &request, Response &response) {
    ...
    if (request.signal_ids_size() == 0) {
        ...
        requested_signal_ids.assign(request.signal_ids().begin(), request.signal_ids().end());
    }
