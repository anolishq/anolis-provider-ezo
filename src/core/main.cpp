...
int main(int argc, char **argv) {
  ...
  try {
    ...
    anolis_provider_ezo::logging::info(
        ...
        anolis_provider_ezo::summarize_config(config));
  ...
  while (true) {
    frame.clear();
    const bool read_ok =
        anolis_provider_ezo::transport::read_frame(std::cin, frame, io_error);
    if (!read_ok) {
      if (io_error.empty()) {
    ...
    if (request.has_hello()) {
      ...
      anolis_provider_ezo::handlers::handle_list_devices(request.list_devices(),
                                                         response);
    } else if (request.has_describe_device()) {
      anolis_provider_ezo::handlers::handle_describe_device(
          request.describe_device(), response);
    } else if (request.has_read_signals()) {
      anolis_provider_ezo::handlers::handle_read_signals(request.read_signals(),
                                                         response);
    } else if (request.has_call()) {
      anolis_provider_ezo::handlers::handle_call(request.call(), response);
    } else if (request.has_get_health()) {
      anolis_provider_ezo::handlers::handle_get_health(request.get_health(),
                                                       response);
    } else {
    ...
    if (!response.SerializeToString(&payload)) {
      anolis_provider_ezo::logging::error(
          "failed to serialize Response protobuf");
      return 4;
    }
