...
anolis_provider_ezo::ProviderConfig make_full_config() {
  ...
  config.devices = {
      anolis_provider_ezo::DeviceSpec{
          "ph0", anolis_provider_ezo::EzoDeviceType::Ph, "Tank pH", 0x63},
      anolis_provider_ezo::DeviceSpec{
          "orp0", anolis_provider_ezo::EzoDeviceType::Orp, "Tank ORP", 0x62},
      anolis_provider_ezo::DeviceSpec{
          "ec0", anolis_provider_ezo::EzoDeviceType::Ec, "Tank EC", 0x64},
      anolis_provider_ezo::DeviceSpec{
          "do0", anolis_provider_ezo::EzoDeviceType::Do, "Tank DO", 0x61},
      anolis_provider_ezo::DeviceSpec{
          "rtd0", anolis_provider_ezo::EzoDeviceType::Rtd, "Tank RTD", 0x66},
      anolis_provider_ezo::DeviceSpec{
          "hum0", anolis_provider_ezo::EzoDeviceType::Hum, "Tank HUM", 0x6F},
  };
  return config;
}
...
anolis_provider_ezo::ProviderConfig make_mismatch_config() {
  ...
  config.devices = {
      anolis_provider_ezo::DeviceSpec{
          "ph0", anolis_provider_ezo::EzoDeviceType::Ph, "Tank pH", 0x63},
      anolis_provider_ezo::DeviceSpec{
          "wrong0", anolis_provider_ezo::EzoDeviceType::Ph, "Wrong", 0x61},
  };
  return config;
}
...
TEST(HandlersTest, WaitReadyAndGetHealthReflectActiveAndExcludedDevices) {
  ...
  anolis_provider_ezo::handlers::handle_wait_ready(
      anolis::deviceprovider::v1::WaitReadyRequest{}, wait_response);
  EXPECT_EQ(wait_response.status().code(),
            anolis::deviceprovider::v1::Status::CODE_OK);
  EXPECT_EQ(wait_response.wait_ready().diagnostics().at("ready"), "true");
  EXPECT_EQ(wait_response.wait_ready().diagnostics().at("active_device_count"),
            "1");
  EXPECT_EQ(
      wait_response.wait_ready().diagnostics().at("excluded_device_count"),
      "1");

  anolis::deviceprovider::v1::Response health_response;
  anolis_provider_ezo::handlers::handle_get_health(
      anolis::deviceprovider::v1::GetHealthRequest{}, health_response);
  EXPECT_EQ(health_response.status().code(),
            anolis::deviceprovider::v1::Status::CODE_OK);
...
TEST(HandlersTest, ListDevicesReturnsOnlyActiveInventoryAcrossFamilies) {
  ...
  EXPECT_EQ(response.list_devices().devices(0).tags().at("hw.bus_path"),
            "mock://unit-test-i2c");
  EXPECT_EQ(response.list_devices().devices(0).tags().at("hw.i2c_address"),
            "0x63");
...
TEST(HandlersTest, DescribeDeviceReturnsDoCapabilitiesAndSafeFunctions) {
  ...
  EXPECT_EQ(response.describe_device().capabilities().signals(0).signal_id(),
            "do.mg_l");
  EXPECT_EQ(response.describe_device().capabilities().signals(1).signal_id(),
            "do.saturation_pct");
  ASSERT_EQ(response.describe_device().capabilities().functions_size(), 3);
  EXPECT_EQ(
      response.describe_device().capabilities().functions(0).function_id(),
      1001U);
  EXPECT_EQ(
      response.describe_device().capabilities().functions(1).function_id(),
      1002U);
  EXPECT_EQ(
      response.describe_device().capabilities().functions(2).function_id(),
      1003U);
}

TEST(HandlersTest,
     ReadSignalsReturnsDoFixedSignalSurfaceWithUnavailableMetadata) {
  anolis_provider_ezo::runtime::reset();
  anolis_provider_ezo::runtime::initialize(make_full_config());
  ...
  EXPECT_EQ(response.read_signals().values(1).metadata().at("unavailable"),
            "true");
}

TEST(HandlersTest,
     ReadSignalsReturnsEcFixedSignalSurfaceWithUnavailableMetadata) {
  anolis_provider_ezo::runtime::reset();
  anolis_provider_ezo::runtime::initialize(make_full_config());
...
TEST(HandlersTest,
     ReadSignalsReturnsHumFixedSignalSurfaceWithUnavailableMetadata) {
  anolis_provider_ezo::runtime::reset();
  anolis_provider_ezo::runtime::initialize(make_full_config());
...
TEST(HandlersTest, CallSetLedByNameReturnsResultsAndUpdatesHealth) {
  ...
  anolis_provider_ezo::handlers::handle_get_health(
      anolis::deviceprovider::v1::GetHealthRequest{}, health_response);
  ASSERT_EQ(health_response.status().code(),
            anolis::deviceprovider::v1::Status::CODE_OK);

  const auto *device_health =
      find_device_health(health_response.get_health(), "ph0");
...
TEST(HandlersTest, CallFindAndSleepSucceed) {
  ...
  EXPECT_EQ(find_response.status().code(),
            anolis::deviceprovider::v1::Status::CODE_OK);
  ...
  EXPECT_EQ(sleep_response.status().code(),
            anolis::deviceprovider::v1::Status::CODE_OK);
